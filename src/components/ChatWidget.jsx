  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/chat", {
        ...
        body: JSON.stringify({ prompt: input }), // must match backend key
      });
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {isOpen ? (
        <div
          ...
          } border border-gray-300 dark:border-gray-700 rounded-lg shadow-lg flex flex-col`}
        >
          ...
          <div className="flex p-2 border-t border-gray-300 dark:border-gray-600">
            ...
            <button
              onClick={sendMessage}
              disabled={loading}
              className="px-4 py-1 bg-blue-500 text-white rounded-r hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Send
            </button>
          </div>
